import { defineComponent } from '@dotdev/design'

export const doc = defineComponent({
  ui: 'doc',
  layer: 'studio',
  semantics: {
    'bg-tint': ['brand-50', 'brand-400/10'],
    'bg-card': 'bg-background',
    'bg-grid-color': 'neutral-500/30',
    'bg-token-preview': 'bg-background',
    'bg-token-padding': ['brand-100', 'brand-400/25'],
    'bg-shape': ['neutral-100', 'neutral-800'],
  },
  utilities: {
    card: `doc-bg-card border border-default radius-lg transition-[background-color,border-color]`,
    'bg-grid': 'bg-[radial-gradient(var(--$ns-doc-bg-grid-color)_1px,transparent_1px)] bg-size-[20px_20px]',
    'code-text': 'text-[0.9em]',
    'header-icon':
      'size-7.5 flex items-center justify-center border border-brand-soft bg-brand-soft text-brand text-base radius-md',
  },
  rules: {
    '.doc-page': `py-10 px-4 lg:px-8 flex gap-10 items-start`,
    '.doc-page__body': `w-full min-w-0`,
    '.doc-page__header': `mb-8`,
    '.doc-page__caption': `flex items-center justify-between gap-4 mb-3`,
    '.doc-page__caption-label': `flex items-center gap-1.5 font-mono text-xs uppercase tracking-wider text-muted`,
    '.doc-page__title': `font-bold text-2xl tracking-tight flex items-center gap-3`,
    '.doc-page__title-icon': `doc-header-icon`,
    '.doc-page__gh': `font-mono text-muted`,
    '.doc-page__desc': `mt-4 text-muted max-w-prose`,
    '.doc-page__divider': `mt-10 mb-10`,
    '.doc-page__example': `scroll-mt-24`,
    '.doc-bg-grid': 'doc-bg-grid',

    '.doc-section': `flex flex-col gap-5 scroll-mt-24`,
    '.doc-section__title': `font-semibold text-sm uppercase tracking-wider text-muted`,

    '.doc-sidebar': `sticky top-24 hidden xl:block w-64 shrink-0 max-h-[calc(100vh-6rem)] overflow-y-auto`,
    '.doc-sidebar__title': `font-mono text-xs uppercase tracking-wider text-muted mb-4`,
    '.doc-sidebar__nav': `flex flex-col gap-1 border-l border-default`,
    '.doc-sidebar__link': `text-sm text-muted -ml-px border-l-2 border-transparent px-3 py-1.5 transition-[color,border-color] hover:text-foreground`,
    '.doc-sidebar__link--active': `border-brand text-foreground font-medium`,

    '.doc-block': `mb-10 mt-4 scroll-mt-24`,
    '.doc-block__header': `mb-4`,
    '.doc-block__title': `text-lg font-semibold mb-2`,
    '.doc-block__desc': `text-muted`,

    '.doc-desc': `whitespace-pre-wrap`,
    '.doc-desc code': `doc-code-text doc-bg-tint px-1.5 py-px radius-sm text-brand`,

    '.doc-card': `doc-card`,
    '.doc-card__header, .doc-card__footer': `border-default p-2 flex justify-between transition-[border-color]`,
    '.doc-card__footer': `border-t`,
    '.doc-card__header': `border-b`,

    '.doc-example__header': `flex gap-1 pb-4 mt-8`,
    '.doc-example__space': `flex-1`,
    '.doc-example__body': `relative`,
    '.doc-example__canvas': `flex`,
    '.doc-example__canvas-inner': `flex flex-1 flex-wrap items-center justify-center gap-6 py-12 px-6`,
    '.doc-example__toggle': `font-mono`,
    '.doc-example__body .doc-example__toggle': `absolute top-1 right-1 z-10`,

    '.doc-code': `max-h-100 h-auto`,
    '.doc-code__raw': `py-4 pr-4`,
    '.doc-code .shiki': `flex text-sm outline-none`,
    '.doc-code .shiki code': `pr-4`,
    '.doc-code .shiki span': `text-(--shiki-light) `,
    '.dark .doc-code .shiki span': `text-(--shiki-dark)`,

    '.doc-tokens': `flex flex-col gap-8 scroll-mt-24 mt-12`,
    '.doc-tokens__header': `flex items-center gap-4`,
    '.doc-tokens__icon': `doc-header-icon`,
    '.doc-tokens__title': `font-semibold text-lg tracking-tight`,
    '.doc-tokens__count': `ml-auto font-mono text-xs text-muted tabular-nums`,
    '.doc-tokens__group': `flex flex-col gap-3`,
    '.doc-tokens__group-label': `flex items-center gap-3 font-mono text-xs uppercase tracking-wider text-muted`,
    '.doc-tokens__group-rule': `flex-1 border-t border-default`,
    '.doc-tokens__group-count': `font-mono text-muted tabular-nums`,
    '.doc-tokens__grid': `grid grid-cols-1 md:grid-cols-3 gap-3`,

    '.doc-token': `flex flex-col radius-lg overflow-hidden border border-default transition-[border-color] hover:border-neutral`,
    '.doc-token__well': `h-20 flex items-center justify-center doc-bg-token-preview`,
    '.doc-token__meta': `flex flex-col gap-1.5 border-t border-default p-2.5`,
    '.doc-token__head': `flex items-start justify-between gap-2`,
    '.doc-token__name': `min-w-0 py-0.5 font-mono text-xs font-medium leading-4 text-foreground line-clamp-2 break-all`,
    '.doc-token__value': `font-mono text-[0.6875rem] leading-4 text-muted break-all`,
    '.doc-token__bg': `size-10 radius-md border border-default`,
    '.doc-token__text': `text-xl font-semibold leading-none`,
    '.doc-token__border': `size-10 radius-md border-2`,
    '.doc-token__ring': `size-9 radius-md doc-bg-shape outline-2 outline-offset-2`,
    '.doc-token__space': `flex items-center`,
    '.doc-token__shape': `size-2.5 radius-xs doc-bg-shape`,
    '.doc-token__padding': `w-full h-full flex doc-bg-token-padding`,
    '.doc-token__content': `w-full h-full doc-bg-shape`,
    '.doc-token__radius': `size-10 doc-bg-shape`,
    '.doc-token__measure': `radius-sm doc-bg-shape shrink-0`,
    '.doc-token__measure--height': `aspect-2/1`,
    '.doc-token__measure--width': `aspect-2/1`,
  },
})
